...
std::shared_ptr<prometheus::Registry> MetricsManager::GetSubsystemRegistry(const std::string& ns) {
        EnsureInitialized();
        if (ns.empty()) {
    ...
    void MetricsManager::SetDefaultLabels(const std::map<std::string,std::string>& labels) {
    ...
    std::map<std::string,std::string> MetricsManager::GetDefaultLabels() {
     ...
     std::map<std::string,std::string> MetricsManager::GetDynamicLabels() {
        ...
        std::map<std::string,std::string> MetricsManager::MergeLabels(const std::map<std::string,std::string>& labels) {
 ...
 void MetricsManager::EnsureInitialized() {
        ...
        if (!initialized_ || !registry_) {
            throw std::runtime_error(
                "MetricsManager not initialized. Call MetricsManager::Init() first."
            );
        }
    }
 ...
 void MetricsManager::ValidateMetricName(const std::string& name) {
        ...
        if (name.size() >= 2 && name[0] == '_' && name[1] == '_') {
            throw std::invalid_argument(
                "Metric name '" + name + "' cannot start with '__' (reserved prefix)"
            );
        }
     ...
     void MetricsManager::ValidateLabels(const std::map<std::string,std::string>& labels) {
        ...
        for (const auto& [key, value] : labels) {
            ...
            if (key.size() >= 2 && key[0] == '_' && key[1] == '_') {
                throw std::invalid_argument(
                    "Label name '" + key + "' cannot start with '__' (reserved prefix)"
                );
            }
     ...
     bool MetricsManager::ValidateLabelValue(const std::string& value) {
        ...
        for (char c : value) {
            if (c < 32 && c != '\t') {  // Reject control chars except tab
                return false;
            }
        }
        return true;
