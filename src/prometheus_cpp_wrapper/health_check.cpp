#include <prometheus_cpp_wrapper/health_check.hpp>

namespace vex::metrics {

void HealthCheck::RegisterHealthMetrics(std::shared_ptr<prometheus::Registry> registry) {
    if (!registry) {
...
void HealthCheck::SetHealthy(bool healthy) {
    if (!health_status_) {
        throw std::runtime_error(
            "Health metrics not registered. Call RegisterHealthMetrics() first."
        );
    }
...
bool HealthCheck::IsRegistered() {
    return health_status_ != nullptr &&
           uptime_gauge_ != nullptr &&
