...
namespace vex
{
    ...
    struct LoggerStats
    {
        ...
        LoggerStats(const LoggerStats& other)
            : records_written(
                other.records_written.load(std::memory_order_relaxed)
              ),
              records_dropped(
                other.records_dropped.load(std::memory_order_relaxed)
              ),
              ...
              contention_events(
                other.contention_events.load(std::memory_order_relaxed)
              )
        {
        }
    ...
    namespace detail
    {
        ...
        inline TimeComponents
          decompose_time(const std::chrono::system_clock::time_point& tp)
        {
        ...
        public:
            ...
            std::string format(
              const TimeComponents& open_time, const TimeComponents& close_time,
              uint32_t sequence
            ) const
            {
                std::string result;
                ...
                for (const auto& part: parts_)
                {
                    ...
                    else
                    {
                        ...
                        append_field(
                          result, field, open_time, close_time, sequence
                        );
                    }
                }
        ...
        private:
            ...
            void parse_format(std::string_view format)
            {
                ...
                for (size_t i = 0; i < format.size(); ++i)
                {
                    if (format[i] == '%' && i + 1 < format.size())
                    {
                        ...
                        {
                            case 'Y':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenYear4
                                             : FieldType::CloseYear4,
                                  4});
                                break;
                            case 'y':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenYear2
                                             : FieldType::CloseYear2,
                                  2});
                                break;
                            case 'M':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenMonth
                                             : FieldType::CloseMonth,
                                  2});
                                break;
                            case 'd':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenDay
                                             : FieldType::CloseDay,
                                  2});
                                break;
                            case 'h':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenHour
                                             : FieldType::CloseHour,
                                  2});
                                break;
                            case 'm':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenMinute
                                             : FieldType::CloseMinute,
                                  2});
                                break;
                            case 's':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenSecond
                                             : FieldType::CloseSecond,
                                  2});
                                break;
                            case 'S':
                                parts_.emplace_back(FormatField {
                                  begin_time ? FieldType::OpenMillis
                                             : FieldType::CloseMillis,
                                  3});
                                break;
                            case 'n':
                                parts_.emplace_back(
                                  FormatField {FieldType::Sequence, 4}
                                );
                                break;
            ...
            void append_field(
              std::string& out, const FormatField& field,
            ...
            ) const
            {
                ...
                switch (field.type)
                {
                    ...
                    case FieldType::CloseMillis:
                        value = close.millisecond;
                        break;
        ...
        private:
            ...
            public:
                ...
                bool try_push(T&& item)
                {
                    const size_t current_write = write_idx_.load(
                ...
                bool try_pop(T& item)
                {
                    const size_t current_read = read_idx_.load(
                      std::memory_order_relaxed
                    );
                    ...
                    read_idx_.store(
                      (current_read + 1) % capacity_, std::memory_order_release
                    );
                    return true;
                }
    ...
    public:
              ...
              sequence_number_(1)
        {
            ...
            queue_   = std::make_unique<detail::MPSCQueue<std::string>>(
              cfg.queue_capacity, cfg.num_shards
            );
    ...
    private:
        ...
        LoggerConfig load_config()
        {
            ...
            cfg.records_threshold = config_->at("records_threshold")
                                      ->get<uint32_t>();
            cfg.time_threshold = std::chrono::seconds(
              config_->at("time_threshold")->get<uint32_t>()
            );
            cfg.file_name_format = config_->at("file_name_format")
                                     ->get<std::string>();
            ...
            cfg.file_mode = (mode == "binary") ? FileMode::Binary
                                               : FileMode::Text;
        ...
        void apply_config(const LoggerConfig& cfg)
        {
            ...
            formatter_ = std::make_unique<detail::FileNameFormatter>(
              cfg.file_name_format
            );
        }
        ...
        void setup_config_observers()
        {
            config_obs_enabled_ = config_manager_->on_replace(
              ...
              [this](const auto& node) {
                  enabled_.store(
                    node->template get<bool>(), std::memory_order_relaxed
                  );
              }
            );
            ...
            config_obs_time_ = config_manager_->on_replace(
              ...
              {
                  std::lock_guard lock(config_mutex_);
                  config_data_.time_threshold = std::chrono::seconds(
                    node->template get<uint32_t>()
                  );
              }
            );
        }
        ...
        void prepare_directories()
        {
            ...
            if (!fs::exists(config_data_.create_path))
            {
                ...
                {
                    LOG_ERROR(
                      "Failed to create path: {}", config_data_.create_path
                    );
            ...
            if (!config_data_.close_path.empty() && !fs::exists(config_data_.close_path))
            {
                ...
                {
                    LOG_ERROR(
                      "Failed to create path: {}", config_data_.close_path
                    );
        ...
        void cleanup_incomplete_files()
        {
            ...
            for (const auto& entry:
                 fs::directory_iterator(config_data_.create_path))
            {
                ...
                try
                {
                    ...
                    auto open_tp = std::chrono::system_clock::time_point(
                    ...
                    auto final_name = formatter_->format(
                      open_time, close_time, sequence_number_++
                    );
                ...
                {
                    LOG_ERROR(
                      "Failed to cleanup incomplete file {}: {}",
                      entry.path().string(),
                      e.what()
                    );
                }
        ...
        void close_current_file()
        {
            ...
            try
            {
                ...
                auto final_name = formatter_->format(
                  open_time, close_time_components, sequence_number_
                );

                std::string old_path = config_data_.create_path +
                                       temp_filename_;
